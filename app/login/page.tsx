...
function LoginForm() {
  ...
  async function resetPassword() {
    if (!email) { setErr("Vul eerst je email in"); return; }
    ...
    try {
      ...
      if (error) throw error;
      setInfo("Check je mail voor de reset-link.");
    } catch (e: any) {
      setErr(e?.message ?? "Reset mislukt");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="fade-in max-w-md mx-auto px-4 py-12">
      <div className="bg-card rounded-md border p-7" style={{ borderColor: "#DCE7F4", boxShadow: "0 8px 24px rgba(11,42,74,.10)" }}>
        <div className="text-[11px] font-bold uppercase text-accent mb-2" style={{ letterSpacing: ".14em" }}>Welkom terug</div>
        <h1 className="font-display text-[36px] text-ink m-0 mb-1" style={{ fontWeight: 400 }}>Inloggen</h1>
        <p className="text-ink2 text-[14px] mb-6">Krijg toegang tot je wallet en watchlist op elk apparaat.</p>
        ...
        <form onSubmit={login} className="space-y-3">
          ...
          <div>
            <label className="block text-[11px] uppercase font-bold text-ink3 mb-1.5" style={{ letterSpacing: ".08em" }}>Wachtwoord</label>
            <input
        ...
        <div className="flex items-center justify-between mt-5 text-[12px]">
          <button
            ...
            className="text-ink3 hover:text-accent transition"
          >
            Wachtwoord vergeten?
          </button>
          <a href={`/signup?next=${encodeURIComponent(next)}`} className="text-accent hover:underline font-semibold">
            Account maken →
          </a>
        </div>
      </div>
